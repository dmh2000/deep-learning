...
class Dataset:
    def __init__(self, train, test, val_frac=0.5, shuffle=True, scale_func=None):
        # compute train/validate split ratio
...
def discriminator(x, reuse=False, alpha=0.2, drop_rate=0., num_classes=10, size_mult=64):
    with tf.variable_scope('discriminator', reuse=reuse):
        ...
        # input layer is batch size x height x width x channels
        # output is batch size x channels
        s = relu7.get_shape()
        ...
        # expression sum(exp(logits-max(logits)) + mx
        # max(real_logits)
        m = tf.reduce_max(real_logits,axis=1,keep_dims=True)
        # exp(real_logits-m)
        e = tf.exp(real_logits - m)
        # sum(e)
        s = tf.reduce_sum(e,axis=1)
        gan_logits = m + tf.log(s * e) + fake_logits
